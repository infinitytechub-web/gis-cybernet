...
export default function ProcessingEnquiryApplications() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const channel = supabase
      .channel("processing-enquiry-realtime")
      .on("postgres_changes", { event: "*", schema: "public", table: "enquiry_applications" }, () => {
        qc.invalidateQueries({ queryKey: ["enquiry-apps-processing"] });
      })
      .subscribe();
  ...
  const { data: applications = [], isLoading } = useQuery({
    ...
    queryKey: ["enquiry-apps-processing"],
    queryFn: async () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("enquiry_applications")
      ...
      if (existing && existing.status !== form.status && (form.status === "approved" || form.status === "rejected")) {
        ...
        if (roledUsers) {
          ...
          await Promise.all(uniqueUserIds.map((uid) =>
            createNotification({ userId: uid, title: `Enquiry ${form.status === "approved" ? "Resolved" : "Rejected"}`, message: `Enquiry for ${existing.applicant_name} has been ${form.status}.`, type: "enquiry", referenceId: editId })
          ));
        }
      }
    },
    onSuccess: () => {
      ...
      toast.success("Enquiry updated"); setEditId(null); setOpen(false);
    },
  ...
  const openReview = (app: any) => {
    ...
    setEditId(app.id); setReviewApp(app); setOpen(true);
  };
  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Pending", value: summary.total, color: "text-lime-600 dark:text-lime-400", border: "border-lime-300 dark:border-lime-700", bg: "bg-lime-50/50 dark:bg-lime-950/20" },
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!v) { setEditId(null); } setOpen(v); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Review Enquiry</DialogTitle></DialogHeader>
          {reviewApp && (
            <div className="grid grid-cols-2 gap-2 text-sm border rounded-md p-3 bg-muted/30">
              ...
              <div><span className="text-muted-foreground">Type:</span> {reviewApp.enquiry_type?.replace("_", " ")}</div>
              ...
              {reviewApp.response && <div className="col-span-2"><span className="text-muted-foreground">Response:</span> {reviewApp.response}</div>}
            </div>
          )}
          <form onSubmit={(e) => { e.preventDefault(); updateMutation.mutate(); }} className="space-y-3">
            ...
            <Button type="submit" className="w-full" disabled={updateMutation.isPending}>{updateMutation.isPending ? "Updating..." : "Update Enquiry"}</Button>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Card><CardContent className="p-0"><div className="overflow-x-auto">
        <Table className="min-w-[700px]">
          <TableHeader><TableRow>
            <TableHead>Applicant</TableHead><TableHead>Subject</TableHead><TableHead>Nationality</TableHead><TableHead>Type</TableHead><TableHead>Status</TableHead><TableHead>Submitted</TableHead><TableHead>Actions</TableHead>
          </TableRow></TableHeader>
          <TableBody>
            {isLoading ? (
              <TableRow><TableCell colSpan={7} className="text-center py-8">Loading...</TableCell></TableRow>
            ) : filtered.length === 0 ? (
              <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No enquiries pending processing</TableCell></TableRow>
