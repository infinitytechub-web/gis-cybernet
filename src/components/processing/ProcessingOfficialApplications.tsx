...
export default function ProcessingOfficialApplications() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const channel = supabase
      .channel("processing-official-realtime")
      .on("postgres_changes", { event: "*", schema: "public", table: "official_applications" }, () => {
        qc.invalidateQueries({ queryKey: ["official-apps-processing"] });
      })
      .subscribe();
  ...
  const { data: applications = [], isLoading } = useQuery({
    ...
    queryKey: ["official-apps-processing"],
    queryFn: async () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("official_applications")
      ...
      if (existing && existing.status !== form.status && (form.status === "approved" || form.status === "rejected")) {
        ...
        if (roledUsers) {
          ...
          await Promise.all(uniqueUserIds.map((uid) =>
            createNotification({ userId: uid, title: `Official Application ${form.status === "approved" ? "Approved" : "Rejected"}`, message: `Official application for ${existing.applicant_name} has been ${form.status}.`, type: "official", referenceId: editId })
          ));
        }
      }
    },
    onSuccess: () => {
      ...
      toast.success("Application updated"); setEditId(null); setOpen(false);
    },
  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Pending", value: summary.total, color: "text-cyan-600 dark:text-cyan-400", border: "border-cyan-300 dark:border-cyan-700", bg: "bg-cyan-50/50 dark:bg-cyan-950/20" },
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!v) { setEditId(null); } setOpen(v); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Review Official Application</DialogTitle></DialogHeader>
          {reviewApp && (
            <div className="grid grid-cols-2 gap-2 text-sm border rounded-md p-3 bg-muted/30">
              ...
              <div><span className="text-muted-foreground">Type:</span> {reviewApp.official_type}</div>
              <div><span className="text-muted-foreground">Nationality:</span> {reviewApp.nationality}</div>
              {reviewApp.reference_number && <div><span className="text-muted-foreground">Ref #:</span> {reviewApp.reference_number}</div>}
              {reviewApp.requesting_entity && <div><span className="text-muted-foreground">Entity:</span> {reviewApp.requesting_entity}</div>}
          ...
          <form onSubmit={(e) => { e.preventDefault(); updateMutation.mutate(); }} className="space-y-3">
            ...
            <Button type="submit" className="w-full" disabled={updateMutation.isPending}>{updateMutation.isPending ? "Updating..." : "Update Application"}</Button>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Card><CardContent className="p-0"><div className="overflow-x-auto">
        <Table className="min-w-[700px]">
          <TableHeader><TableRow>
            <TableHead>Applicant</TableHead><TableHead>Ref #</TableHead><TableHead>Nationality</TableHead><TableHead>Type</TableHead><TableHead>Status</TableHead><TableHead>Submitted</TableHead><TableHead>Actions</TableHead>
          </TableRow></TableHeader>
          <TableBody>
            {isLoading ? (
              <TableRow><TableCell colSpan={7} className="text-center py-8">Loading...</TableCell></TableRow>
            ) : filtered.length === 0 ? (
              <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No official applications pending processing</TableCell></TableRow>
