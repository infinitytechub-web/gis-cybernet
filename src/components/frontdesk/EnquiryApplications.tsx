...
export default function EnquiryApplications() {
  const { user } = useAuth();
  ...
  const [form, setForm] = useState({
    applicant_name: "", passport_number: "", nationality: "", enquiry_type: "general",
    subject: "", response: "", purpose: "", notes: "", status: "submitted",
  ...
  useEffect(() => {
    const channel = supabase
      .channel("frontdesk-enquiry-realtime")
      .on("postgres_changes", { event: "*", schema: "public", table: "enquiry_applications" }, () => {
        qc.invalidateQueries({ queryKey: ["enquiry-applications"] });
      })
      .subscribe();
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editId) {
        ...
        const { error } = await supabase.from("enquiry_applications").update(payload).eq("id", editId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("enquiry_applications").insert(payload);
        if (error) throw error;
      }
      if (editId && previousStatus !== form.status && (form.status === "approved" || form.status === "rejected")) {
        ...
        if (roledUsers) {
          ...
          await Promise.all(uniqueUserIds.map((uid) =>
            createNotification({ userId: uid, title: `Enquiry ${form.status === "approved" ? "Resolved" : "Rejected"}`, message: `Enquiry for ${form.applicant_name} has been ${form.status}.`, type: "enquiry", referenceId: editId })
          ));
        }
      }
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["enquiry-applications"] }); toast.success(editId ? "Enquiry updated" : "Enquiry created"); resetForm(); },
    onError: (e: any) => toast.error(e.message),
  });

  const resetForm = () => {
    setForm({ applicant_name: "", passport_number: "", nationality: "", enquiry_type: "general", subject: "", response: "", purpose: "", notes: "", status: "submitted", phone: "", home_address: "", gender: "", marital_status: "", foreign_address: "", date_of_birth: "", next_of_kin: "", emergency_contact: "", street_name: "", nearest_landmark: "" });
  ...
  const openEdit = (app: any) => {
    setForm({
      ...
      nationality: app.nationality, enquiry_type: app.enquiry_type,
      subject: app.subject || "", response: app.response || "",
  ...
  const filtered = applications.filter((a: any) => {
    const matchesSearch = a.applicant_name.toLowerCase().includes(search.toLowerCase()) || (a.subject || "").toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total", value: summary.total, color: "text-lime-600 dark:text-lime-400", border: "border-lime-300 dark:border-lime-700", bg: "bg-lime-50/50 dark:bg-lime-950/20" },
          ...
          { label: "Resolved", value: summary.approved, color: "text-emerald-600 dark:text-emerald-400", border: "border-emerald-300 dark:border-emerald-700", bg: "bg-emerald-50/50 dark:bg-emerald-950/20" },
      ...
      <div className="flex gap-2">
        ...
        <Dialog open={open} onOpenChange={(v) => { if (!v) resetForm(); setOpen(v); }}>
          <DialogTrigger asChild><Button className="gap-1"><Plus className="h-4 w-4" /> New Enquiry</Button></DialogTrigger>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{editId ? "Edit" : "New"} Enquiry</DialogTitle></DialogHeader>
            <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(); }} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><Label>Enquiry Type</Label>
                  <Select value={form.enquiry_type} onValueChange={(v) => setForm({ ...form, enquiry_type: v })}>
                    ...
                    <SelectContent>{ENQUIRY_TYPES.map((t) => <SelectItem key={t} value={t}>{t.replace("_", " ").replace(/^\w/, c => c.toUpperCase())}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
              ...
              {editId && (
                <>
                  <div><Label>Response</Label><Textarea value={form.response} onChange={(e) => setForm({ ...form, response: e.target.value })} rows={3} /></div>
              ...
              <Button type="submit" className="w-full" disabled={saveMutation.isPending}>{saveMutation.isPending ? "Saving..." : editId ? "Update" : "Submit Enquiry"}</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <Card><CardContent className="p-0"><div className="overflow-x-auto">
        <Table className="min-w-[700px]">
          <TableHeader><TableRow>
            <TableHead>Applicant</TableHead><TableHead>Subject</TableHead><TableHead>Nationality</TableHead><TableHead>Type</TableHead><TableHead>Status</TableHead><TableHead>Date</TableHead><TableHead>Actions</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={app.id}>
                ...
                <TableCell><Button variant="ghost" size="icon" onClick={() => openEdit(app)}><Edit className="h-4 w-4" /></Button></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
