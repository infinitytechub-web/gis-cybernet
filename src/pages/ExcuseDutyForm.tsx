...
import { FilePlus2, FileDown, FileText, Activity } from "lucide-react";
...
export default function ExcuseDutyForm() {
  ...
  const { data: profile } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return null;
      const { data } = await supabase.from("profiles").select("id, first_name, last_name, staff_id, departments(name)").eq("user_id", user.id).maybeSingle();
      return data;
    },
    enabled: !!user,
  });
  ...
  const submit = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Excuse Duty Form submitted");
  ...
  const exportPDF = (entry?: any) => {
    ...
    doc.text("Ghana Immigration Service", 105, y, { align: "center" });
    y += 12;
    const lines = [
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><FileText className="h-4 w-4" /> New submission</CardTitle><CardDescription className="text-xs">Forms route to the Health Lab for review.</CardDescription></CardHeader>
