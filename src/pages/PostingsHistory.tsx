...
export default function PostingsHistory() {
  ...
  const doPrint = () => {
    ...
    w.document.write(`<html><head><title>Transfer History</title><style>body{font-family:system-ui;padding:24px}table{width:100%;border-collapse:collapse;font-size:11px}th,td{border:1px solid #ccc;padding:6px}th{background:#f5f5f5}</style></head><body><h1>Staff Transfer History</h1><table><thead><tr>${headers.map(h => `<th>${h}</th>`).join("")}</tr></thead><tbody>${exportRows.map(r => `<tr>${r.map(c => `<td>${c}</td>`).join("")}</tr>`).join("")}</tbody></table></body></html>`);
