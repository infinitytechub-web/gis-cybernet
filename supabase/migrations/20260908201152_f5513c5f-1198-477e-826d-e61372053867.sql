  ...
  allowed AS (
    ...
    WHERE u.is_active
      AND CASE
        ...
        WHEN v_ctx.scope = 'own_subtree' THEN u.id IN (SELECT s2.unit_id FROM sub s2)
        WHEN v_ctx.scope IN ('own_unit', 'shift') THEN u.id = v_ctx.org_unit_id
        WHEN v_ctx.scope = 'self' THEN u.id = v_ctx.org_unit_id
        ELSE false
      END
  )
  SELECT a.unit_id, a.unit_name, a.unit_type,
    (SELECT count(*)::integer FROM public.profiles p WHERE p.org_unit_id = a.unit_id)
  FROM allowed a
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'totals', jsonb_build_object(
      ...
      'positions', (SELECT count(*) FROM public.org_positions op WHERE op.org_unit_id = v_unit AND op.is_active),
      'vacancies', (SELECT count(*) FROM public.org_positions op WHERE op.org_unit_id = v_unit AND op.is_active AND op.holder_profile_id IS NULL),
      'portal_reached', (
    ...
    'officers', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'position_title', (
          SELECT op.title FROM public.org_positions op
          WHERE op.holder_profile_id = s.id AND op.is_active
    ...
    'vacancies', COALESCE((
      ...
      WHERE op.org_unit_id = v_unit AND op.is_active AND op.holder_profile_id IS NULL
    ), '[]'::jsonb),
    'posted_positions', COALESCE((
      ...
      FROM public.org_positions op
      JOIN public.profiles p ON p.id = op.holder_profile_id
      LEFT JOIN public.ranks rk ON rk.id = p.rank_id
      WHERE op.org_unit_id = v_unit AND op.is_active
    ), '[]'::jsonb)
  ) INTO v_result;
