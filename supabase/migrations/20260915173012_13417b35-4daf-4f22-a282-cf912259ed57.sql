ALTER TABLE public.org_units
  ADD COLUMN IF NOT EXISTS authorised_strength integer;

COMMENT ON COLUMN public.org_units.authorised_strength IS
  'Approved establishment headcount for this unit. Vacancies = authorised_strength - posted officers.';
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'totals', jsonb_build_object(
      ...
      -- Establishment figure shown on the dashboard: the approved strength when
      -- it is set, otherwise fall back to the count of named appointments.
      'positions', GREATEST((SELECT authorised FROM establishment), (SELECT count(*)::int FROM positions)),
      -- Headcount gap against the approved strength, never negative.
