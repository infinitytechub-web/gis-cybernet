...
BEGIN
  ...
  ranked AS (
    ...
    WHERE ur.role::TEXT IN ('oic', '2ic')
      AND (
        is_command
        -- The OIC (Sector Commander) authorises across all departments,
        -- so they must always be visible regardless of the viewer's
        -- department scoping. 2IC continues to be department-scoped.
        OR ur.role::TEXT = 'oic'
        OR EXISTS (
          SELECT 1
  ...
  WHERE ranked.rn = 1
  ORDER BY
    CASE ranked.p_role WHEN 'oic' THEN 0 WHEN '2ic' THEN 1 ELSE 2 END,
    ranked.p_last_name,
    ranked.p_first_name
