ALTER TABLE public.biometric_reminder_settings
...
BEGIN
  ...
  UPDATE public.biometric_reminder_settings s
     SET enabled = COALESCE((_patch->>'enabled')::boolean, s.enabled),
         ...
         updated_at = now(),
         updated_by = auth.uid()
   WHERE s.id = (SELECT id FROM public.biometric_reminder_settings ORDER BY created_at LIMIT 1)
   RETURNING * INTO _row;

  INSERT INTO public.webauthn_audit (event, user_id, detail, actor_id)
  VALUES ('policy_change', auth.uid(),
          left(format('Biometric reminder settings updated: %s', _patch::text), 500),
          auth.uid());
...
REVOKE ALL ON FUNCTION public.biometric_reminder_update_settings(jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.biometric_reminder_update_settings(jsonb) TO authenticated, service_role;

-- ============ Reminder run (called by the scheduled mailer) ============
...
DECLARE
  ...
  _considered int := 0;
BEGIN
  -- Only the service role (scheduler) or an administrator may run reminders.
  ...
  -- Respect the configured send hour unless forced (manual "send now").
  ...
  -- Inside the grace period, only start reminding within the lead window.
  ...
  FOR _rec IN
    SELECT p.user_id, p.full_name, p.email, p.staff_id
  ...
  LOOP
    ...
    IF _s.notify_email AND COALESCE(_rec.email, '') <> '' THEN
      ...
      RETURNING id INTO _rec.staff_id;  -- reuse variable slot for the log id

      _emails := _emails || jsonb_build_object(
        'log_id', _rec.staff_id,
...
-- ============ Mailer callback ============
CREATE OR REPLACE FUNCTION public.biometric_reminder_mark(_log_id uuid, _ok boolean, _detail text DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF NOT (public.has_role(auth.uid(), 'admin') OR auth.role() = 'service_role') THEN
    RAISE EXCEPTION 'Not authorised to update reminder delivery state.';
  END IF;

  UPDATE public.biometric_reminder_log
     SET status = CASE WHEN _ok THEN 'sent' ELSE 'failed' END,
         detail = left(COALESCE(_detail, ''), 300)
   WHERE id = _log_id;
END;
$$;

REVOKE ALL ON FUNCTION public.biometric_reminder_mark(uuid, boolean, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.biometric_reminder_mark(uuid, boolean, text) TO authenticated, service_role;