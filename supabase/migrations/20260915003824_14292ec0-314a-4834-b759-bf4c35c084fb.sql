...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS _import_subtree (id uuid primary key) ON COMMIT DROP;
  DELETE FROM _import_subtree;
  ...
  INSERT INTO _import_subtree (id) SELECT id FROM subtree;
  ...
  LOOP
    ...
    v_first := upper(btrim(coalesce(v_row.payload->>'first_name','')));
    v_last  := upper(btrim(coalesce(v_row.payload->>'last_name','')));

    -- Prefer an officer already posted inside the command being imported into.
    SELECT id INTO v_profile_id FROM public.profiles
    WHERE upper(btrim(first_name)) = v_first
      AND upper(btrim(last_name)) = v_last
      AND org_unit_id IN (SELECT id FROM _import_subtree)
    ORDER BY (status = 'active') DESC
    LIMIT 1;

    -- Otherwise only match a name found elsewhere when it is unambiguous
    -- service-wide, so a same-named officer in another command is never
    -- silently re-posted into this one.
    IF v_profile_id IS NULL THEN
      SELECT count(*) INTO v_match_count FROM public.profiles
      WHERE upper(btrim(first_name)) = v_first
        AND upper(btrim(last_name)) = v_last;
    ...
    IF v_profile_id IS NOT NULL THEN
      UPDATE public.profiles SET
        ...
        org_unit_id = coalesce(v_unit_id, org_unit_id, v_imp.target_org_unit_id),
    ...
    ELSE
      ...
      INSERT INTO public.profiles (
        ...
        v_rank_id, coalesce(v_unit_id, v_imp.target_org_unit_id), nullif(v_unit_name,''),
  ...
  -- attendance, leave and audit history survives. The posting itself is kept so
  -- reactivating them restores them to the same command.
  WITH retired AS (
    UPDATE public.profiles p
      SET status = 'inactive', updated_at = now()
    WHERE p.org_unit_id IN (SELECT id FROM _import_subtree)
