...
RETURNS boolean LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$
  SELECT public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'oic') OR public.has_role(auth.uid(), '2ic') OR public.has_role(auth.uid(), 'staff_officer') OR public.has_role(auth.uid(), 'supervisor') OR public.has_role(auth.uid(), 'me_officer');
$$;
...
BEGIN
  ...
  INSERT INTO public.me_approvals (record_type, record_id, workflow_key, current_step, status, requested_by, org_unit_id, classification, due_date) VALUES (_record_type, _record_id, coalesce(_workflow_key,'default'), 1, 'pending', auth.uid(), v_unit, coalesce(v_class,'internal'), current_date + 5) RETURNING id INTO v_id;
...
DECLARE a public.me_approvals; v_total int; v_new_status text; v_record_status text; v_resource_status text;
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
...
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  SELECT jsonb_build_object('generated_at', now(), 'filters', jsonb_build_object('region', _region, 'period_id', _period_id, 'department_id', _department_id), 'objectives', (SELECT jsonb_build_object('total', count(*), 'active', count(*) FILTER (WHERE status IN ('active','approved')), 'avg_score', round(coalesce(avg(performance_score), 0), 2)) FROM public.me_objectives o WHERE archived_at IS NULL AND public.me_can_view(o.classification, o.org_unit_id) AND (_region IS NULL OR o.region = _region) AND (_department_id IS NULL OR o.department_id = _department_id)), 'programs', (SELECT jsonb_build_object('total', count(*), 'on_track', count(*) FILTER (WHERE health = 'on_track'), 'at_risk', count(*) FILTER (WHERE health = 'at_risk'), 'critical', count(*) FILTER (WHERE health = 'critical')) FROM public.me_programs g WHERE archived_at IS NULL AND public.me_can_view(g.classification, g.org_unit_id) AND (_region IS NULL OR g.region = _region) AND (_department_id IS NULL OR g.department_id = _department_id)), 'projects', (SELECT jsonb_build_object('total', count(*), 'active', count(*) FILTER (WHERE status = 'active'), 'completed', count(*) FILTER (WHERE status = 'completed'), 'delayed', count(*) FILTER (WHERE status = 'delayed'), 'on_track', count(*) FILTER (WHERE health = 'on_track'), 'at_risk', count(*) FILTER (WHERE health = 'at_risk'), 'critical', count(*) FILTER (WHERE health = 'critical'), 'avg_complete', round(coalesce(avg(percent_complete),0), 2)) FROM public.me_projects pr WHERE archived_at IS NULL AND public.me_can_view(pr.classification, pr.org_unit_id) AND (_region IS NULL OR pr.region = _region) AND (_department_id IS NULL OR pr.department_id = _department_id)), 'field_reports', (SELECT jsonb_build_object('total', count(*), 'submitted', count(*) FILTER (WHERE fr.status = 'submitted'), 'approved', count(*) FILTER (WHERE fr.status IN ('verified','approved')), 'pending_review', count(*) FILTER (WHERE fr.status IN ('submitted','returned'))) FROM public.me_field_reports fr WHERE public.me_can_view(fr.classification, fr.org_unit_id) AND (_region IS NULL OR fr.region = _region) AND (_department_id IS NULL OR fr.department_id = _department_id)), 'resources', (SELECT jsonb_build_object('allocations', count(*), 'quantity', coalesce(sum(quantity),0), 'in_use', count(*) FILTER (WHERE status = 'in_use')) FROM public.me_resource_allocations r WHERE public.me_can_view('internal', r.org_unit_id) AND (_region IS NULL OR r.region = _region)), 'budget', (SELECT jsonb_build_object('approved', coalesce(sum(coalesce(b.revised_amount, b.approved_amount)),0), 'committed', coalesce(sum(b.committed_amount),0), 'spent', coalesce((SELECT sum(amount) FROM public.me_expenditures e WHERE e.expenditure_type='actual'),0), 'records', count(*), 'pending', count(*) FILTER (WHERE b.status='submitted')) FROM public.me_budgets b WHERE public.me_can_view(b.classification, b.org_unit_id) AND (_region IS NULL OR b.region = _region)), 'procurement', (SELECT jsonb_build_object('total', count(*), 'pending', count(*) FILTER (WHERE status='submitted'), 'approved', count(*) FILTER (WHERE status='approved'), 'estimated_value', coalesce(sum(estimated_cost),0)) FROM public.purchase_requisitions r WHERE requested_by = auth.uid() OR public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'oic') OR public.has_role(auth.uid(),'2ic') OR public.has_role(auth.uid(),'procurement_officer') OR public.has_role(auth.uid(),'supervisor')), 'measures', (SELECT jsonb_build_object('total', count(*), 'kpis', count(*) FILTER (WHERE measure_class = 'kpi'), 'indicators', count(*) FILTER (WHERE measure_class = 'indicator')) FROM public.me_measures m WHERE archived_at IS NULL AND public.me_can_view(m.classification, m.org_unit_id)), 'achievement', '{}'::jsonb, 'risks', (SELECT jsonb_build_object('open', count(*) FILTER (WHERE status <> 'closed'), 'high', count(*) FILTER (WHERE risk_score >= 15 AND status <> 'closed'), 'max_score', coalesce(max(risk_score) FILTER (WHERE status <> 'closed'), 0)) FROM public.me_risks rk WHERE public.me_can_view(rk.classification, rk.org_unit_id) AND (_region IS NULL OR rk.region = _region)), 'incidents', (SELECT jsonb_build_object('open', count(*) FILTER (WHERE status NOT IN ('resolved','closed')), 'critical', count(*) FILTER (WHERE severity='critical' AND status NOT IN ('resolved','closed'))) FROM public.me_incidents ic WHERE public.me_can_view(ic.classification, ic.org_unit_id) AND (_region IS NULL OR ic.region = _region)), 'approvals', (SELECT jsonb_build_object('pending', count(*) FILTER (WHERE status IN ('pending','escalated')), 'approved', count(*) FILTER (WHERE status='approved')) FROM public.me_approvals ap WHERE public.me_can_view(ap.classification, ap.org_unit_id)), 'generated_for', auth.uid()) INTO res;
  RETURN res;
END;
$$;
