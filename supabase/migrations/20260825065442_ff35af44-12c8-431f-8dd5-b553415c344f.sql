...
BEGIN
  ...
  UNION ALL
  -- MFA challenges (verification / enrolment step-up outcomes)
  SELECT m.id, m.created_at, 'mfa'::text,
         ...
         m.staff_id,
         (SELECT p.full_name FROM public.profiles p WHERE p.id = m.user_id),
         NULL::text,
         m.ip_address,
  ...
  UNION ALL
  -- Session actions (device-cap revocations, forced sign-outs)
  SELECT s.id, s.created_at, 'session'::text,
         ...
         (SELECT p.full_name FROM public.profiles p WHERE p.id = s.target_user_id),
         (SELECT p.full_name FROM public.profiles p WHERE p.id = s.actor_id),
         s.ip_address,
  ...
  UNION ALL
  -- Hash-chained security audit log: MFA / lockout related entries
