-- Returns the OIC / 2IC officers visible to the current user, deduplicated
-- on profile id. Encapsulates: role filter, department scoping, name search,
-- and "highest role wins" selection (oic preferred over 2ic) so the UI can
-- never receive duplicate rows even when an officer:
--   * holds BOTH `oic` and `2ic` user_roles, OR
--   * is attached to MULTIPLE departments visible to the viewer.
...
BEGIN
  ...
  -- Officers + their highest applicable role. ROW_NUMBER ensures we keep
  -- exactly one row per profile (oic preferred over 2ic).
  ranked AS (
    SELECT
      p.id,
      p.first_name,
  ...
  SELECT id, first_name, last_name, rank_abbrev, department_name, role
  FROM ranked
  WHERE rn = 1
