...
BEGIN
  ...
  fuel AS (
    SELECT f.vehicle_id,
           COALESCE(SUM(CASE WHEN f.event = 'reading' AND f.delta_litres < 0 THEN -f.delta_litres END), 0) AS litres_used,
           COALESCE(SUM(CASE WHEN f.event = 'refuel' THEN GREATEST(f.delta_litres, 0) END), 0) AS litres_refuelled,
           COUNT(*) FILTER (WHERE f.event = 'refuel') AS refuels,
           COUNT(*) FILTER (WHERE f.event = 'drain') AS suspected_drains
  ...
  gf AS (
    SELECT e.vehicle_id, e.event_type, g.restricted
...
REVOKE ALL ON FUNCTION public.fleet_dashboard(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fleet_dashboard(integer) TO authenticated;