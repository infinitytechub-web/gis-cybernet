...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (ids.length === 0) return json({ ...(scan as any), emailed: 0, reason: "no admins" });
    ...
    if (recipients.length === 0) return json({ ...(scan as any), emailed: 0, reason: "no admin emails" });
    ...
    return json({ ...(scan as any), emailed });
  } catch {
